...
| Champ | Valeur |
|---|---|
| Date | 25 août 2026 (dernière révision : 17 septembre 2026, voir journal ci-dessous, entrée 15) |
...
| Format | Ce document (synthèse de lecture) + `sbom/sillon-sbom-cyclonedx.json` (référence machine-lisible, CycloneDX 1.6, 818 composants, validée sans erreur contre le schéma officiel) |
...
| 15 | **17 septembre 2026** — Resynchronisation de `sbom/sillon-sbom-cyclonedx.json` avec le constat 13 : ajout des 80 composants `python3-geopandas` (univers image d'exécution), correction des 13 dérives de version associées, mise à jour des étiquettes de version des composants `sillon-server` (0.1.36 → 0.1.39), `sillon-orchestrateur` (0.1.13 → 0.1.15), `sillon-image-execution` (0.1.2 → 0.1.3) et `sillon-tutoriel` (0.2.0 → 0.2.3) et de toutes leurs références croisées (`dependencies`, `purl`) — aucune nouvelle dépendance Debian constatée pour ces trois derniers au-delà de l'étiquette de version elle-même. Ajout des 14 CVE non corrigées de `gdal`/`netcdf` (constat 13) à la section `vulnerabilities` au format VEX CycloneDX (`analysis.state: not_affected`, `justification: protected_by_mitigating_control`), avec le même raisonnement que documenté dans ce document. 738 → **818 composants**. Revalidé sans erreur contre le schéma officiel CycloneDX 1.6 (`bom-1.6.schema.json`), aucune référence pendante (`dependencies`/`vulnerabilities.affects`), aucun doublon de `bom-ref`. |
| 16 | **17 septembre 2026** — `sillon-image-execution` 0.1.3 → **0.1.4** : vendoring de `python3-scipy`/`python3-sklearn`/`python3-statsmodels`/`python3-seaborn` (constat 14, §7). 458 → **466 paquets** (+8, `python3-scipy` déjà présent via `python3-geopandas`), image `image-execution.tar` 1 289 Mo → **1 399 Mo**. `sillon-server` 0.1.39 → **0.1.40** : `librairies-scripts.json` mis à jour en miroir (4 nouvelles entrées). Construit et testé localement (`docker build`, options de substitution habituelles), puis **installé et vérifié en conditions réelles sur `192.168.122.80`** (mise à niveau depuis 0.1.3, `podman load` réussi). |
| 17 | **17 septembre 2026** — `sillon-tutoriel` 0.2.3 → **0.2.4** : deux nouveaux scripts d'exemple, `exemple_2.7_typologie_scikit_learn.py` (`StandardScaler` + `KMeans`, typologie non supervisée des 34 868 communes en 4 profils : pôles urbains, communes standards, grandes communes rurales, communes de montagne) et `exemple_2.8_regression_statsmodels_seaborn.py` (régression OLS `statsmodels` de `log(population)` sur densité/altitude/superficie, R² = 0,342, visualisations `seaborn` : matrice de corrélation et droite de régression), plus les corrigés `exercice_2.7.py` (méthode du coude) et `exercice_2.8.py` (diagnostic des résidus extrêmes). Dépendance minimale relevée à `sillon-image-execution (>= 0.1.4)` (entrée 16). Construit et testé localement (mêmes calculs rejoués sur le CSV source, hors SILLON, avant intégration), puis **installé et vérifié en conditions réelles sur `192.168.122.80`** (mise à niveau depuis 0.2.3, jobs n°21 à 28 tous `termine`, résultats des deux nouveaux scripts — jobs n°26/27 — téléchargés et identiques à la vérification hors SILLON). |
...
| Paquets Debian résolus — univers image d'exécution | 466 (paquets source uniques à recompter à la prochaine résolution complète) — +8 le 17/09/2026 pour `scipy`/`scikit-learn`/`statsmodels`/`seaborn` (constat 14), 458 auparavant |
...
| Images conteneur figées | 1 (`image-execution.tar`, **1 399 Mo** — 1 289 Mo avant l'ajout de scipy/scikit-learn/statsmodels/seaborn, constat 14) |
...
| Composants totaux (fichier CycloneDX) | 818 (+7 le 15/09/2026 : `unattended-upgrades` et sa fermeture transitive, constat 11 ; +80 le 17/09/2026 : `python3-geopandas` et sa fermeture, constat 13, resynchronisation entrée 15) — **+8 du 17/09/2026 pour scipy/scikit-learn/statsmodels/seaborn (constat 14) pas encore repris, voir §8** |
...
| Dernière vérification CVE effective (scipy/scikit-learn/statsmodels/seaborn, univers image d'exécution) | **17/09/2026** — voir constat 14, §7 : 3 CVE ouvertes trouvées (2 sur `scikit-learn`, 1 sur `joblib`), aucune atteignable dans le modèle de menace de SILLON |
...
|---|---|---|---|---|---|
| `sillon-server` | **0.1.40** | amd64 | postgresql-17, nginx, openssl, sudo, fail2ban, unattended-upgrades | — | PostgREST 14.8, 9 libs JS, DSFR 1.15.2 |
...
| `sillon-image-execution` | **0.1.4** | amd64 | *(univers séparé, §4)* | sillon-worker | image podman figée (**1 399 Mo**, 1 289 Mo avant l'ajout de scipy/scikit-learn/statsmodels/seaborn), base épinglée par digest |
| `sillon-tutoriel` | **0.2.4** | all | python3 | sillon-server, sillon-orchestrateur, sillon-worker, sillon-image-execution (>= 0.1.4) | — |
...
**17 septembre 2026 (constat 14)** : `sillon-image-execution` 0.1.4 (`python3-scipy`/`python3-sklearn`/`python3-statsmodels`/`python3-seaborn`), `sillon-server` 0.1.40 (`librairies-scripts.json` mis à jour) et `sillon-tutoriel` 0.2.4 (nouveaux exemple/exercices 2.7 et 2.8) construits en local (`docker build`, `podman` toujours indisponible sur la machine de développement, mêmes options de substitution que `build/build.sh`), puis **installés en mise à niveau sur la même VM `192.168.122.80`** (déjà utilisée au constat 13, donc podman/`sillon-worker`/`sillon-image-execution` déjà en place cette fois). `sillon-tutoriel` relève sa dépendance minimale à `sillon-image-execution (>= 0.1.4)` pour la même raison qu'au constat 13. Les 8 scripts d'exemple du tutoriel (dont les deux nouveaux, `exemple_2.7_typologie_scikit_learn.py` et `exemple_2.8_regression_statsmodels_seaborn.py`) ont été exécutés réellement par ce worker mis à niveau, contre la base PostgreSQL réelle du compte `demo@sillon.local` (jobs n°21 à 28, tous `termine`) : résultats téléchargés (`profil_clusters.csv`, `regression_population.txt`, `residus_par_departement.csv`) et comparés à la vérification préalable hors SILLON (mêmes calculs rejoués directement sur le CSV source, sans base PostgreSQL) — identiques (R² = 0,342 ; 4 profils de clustering avec les mêmes effectifs 156/28 824/1 770/3 984). Même limite qu'au constat 13 : cette VM ne remplace pas `192.168.122.114` (§2) comme référence de résolution des annexes A/B/C.
...
### 6.1 État des correctifs de sécurité amont (vérifié le 19 août 2026, corrigé le même jour ; revérifié le 15 septembre 2026)
...
**Revérification du 15/09/2026** (interrogation directe de l'API OSV.dev sur les 8 bibliothèques npm de ce tableau, plus les registres d'advisories GitHub pour PostgREST) : aucune régression, le tableau ci-dessous reste exact sans modification — les composants vendorisés eux-mêmes ne sont pas en cause dans les constats de sécurité du 15/09/2026 (voir constat 11, §7), qui concernent exclusivement l'univers hôte (`postgresql-17`, `nginx`), hors périmètre de ce tableau par construction (§2).
...
| 13 | **16 septembre 2026** — Vendoring de `python3-geopandas` (cartographie SIG en script, §5.4/§7.7) dans `sillon-image-execution` (0.1.2 → **0.1.3**), à la demande explicite d'une évolution fonctionnelle du tutoriel. Simulation d'installation (`apt-get install -s`) avant construction : **+154 paquets candidats**, dont GDAL et toute sa pile de pilotes de formats (HDF5, NetCDF, MariaDB, ODBC, Poppler/PDF, NSS, SpatiaLite, libcurl) — après dédoublonnage avec les paquets déjà présents, **+80 paquets réellement ajoutés** à l'image (378 → **458**, Annexe B ; taille de l'image 384 Mo → **1 289 Mo**), et 13 paquets déjà présents mis à niveau par dérive normale des dépôts Debian entre le 15 et le 16/09/2026 (openssl, python3.13, glib2.0…), sans lien avec geopandas. Vérification CVE de cet ajout sur le Debian Security Tracker, **80/80 nouveaux paquets couverts** (chacun de leurs paquets source respectifs interrogé individuellement) — `geopandas`/`shapely`/`pyproj`/`pyogrio` eux-mêmes n'ont **aucune CVE historique** ; parmi les autres, l'essentiel n'a **aucun problème ouvert** au tracker (`geos`, `proj`, `unixodbc`, `libkml`, `spatialite`, `xerces-c`, `ngtcp2`, tous les codecs image AV1/HEIF sauf `libheif` lui-même, etc.) : `gdal` porte **8 CVE non corrigées** côté Debian (dont **CVE-2026-49014**, exécution de code à distance par dépassement de tampon dans le pilote netCDF sur fichier forgé, et 5 CVE HDF-EOS/HDF4 similaires) et `netcdf` **6 CVE non corrigées** (dont plusieurs RCE par nom d'attribut/dimension forgé dans un fichier HDF5/NetCDF) — toutes désamorcées par le même raisonnement que le constat 12 : un script utilisateur dispose déjà d'une exécution de code arbitraire dans son propre conteneur isolé, donc une RCE déclenchée par un fichier qu'il choisit lui-même de faire lire par son propre script ne lui donne rien qu'il n'ait déjà. `mariadb` porte 14 CVE non corrigées, mais elles visent toutes le **serveur** MySQL/MariaDB (`mysqld`) — seul le paquet **client** (`libmariadb3`) est vendorisé ici, composant non présent, donc non concerné. `curl`/`poppler`/`nss`/`gnupg2`/`zlib`/`libheif`/`uriparser` : CVE mineures ou déjà présentes avant cet ajout (curl, via `r-cran-tidyverse`/`httr`, constat 3), sans lien avec geopandas. | **Vendorisé, déployé et vérifié en conditions réelles (16/09/2026)** | `sillon-image-execution`, `sillon-worker`, `sillon-tutoriel`, `sillon-server` (`librairies-scripts.json`) | Image reconstruite (`docker build`, `podman` indisponible sur la machine de développement — mêmes options de substitution que `build/build.sh`, voir constat 1) puis vérifiée d'abord en local dans les conditions exactes du sandbox (`--read-only --tmpfs /tmp --user 1000:1000`) : reconstitution de géométries, reprojection Lambert-93, `dissolve()`, jointure spatiale (`sjoin`) et export PNG/CSV tous fonctionnels sur le jeu de données réel du tutoriel. **Installée et revérifiée sur la VM de test `192.168.122.80`** (podman, `sillon-worker` et `sillon-image-execution` mis en place pour la première fois sur cette VM) : les 5 scripts d'exemple du tutoriel, dont `exemple_2.5_geopandas.py`, exécutés réellement par ce worker contre la base PostgreSQL du compte `demo@sillon.local`, tous terminés avec succès (résultats identiques à la vérification locale). Les 80 nouveaux paquets ont chacun été couverts par la vérification CVE (via leur paquet source). SBOM CycloneDX resynchronisé le 17/09/2026 (journal, entrée 15). Reste à faire : report de ces versions dans les annexes A/B/C, résolues sur une VM différente (`192.168.122.114`, §2) |
| 14 | **17 septembre 2026** — Vendoring de `python3-scipy`, `python3-sklearn`, `python3-statsmodels` et `python3-seaborn` (typologie de communes par apprentissage automatique et régression/statistiques inférentielles en script, §5.4/§7.7) dans `sillon-image-execution` (0.1.3 → **0.1.4**), à la demande explicite d'un utilisateur. Simulation d'installation (`apt-get install -s`, conteneur `debian:13-slim` reconstitué à l'identique du socle déjà vendorisé) : `python3-scipy` déjà satisfait en dépendance transitive de `python3-geopandas` (constat 13), donc **seulement 8 paquets réellement ajoutés** — `python3-sklearn`, `python3-sklearn-lib`, `python3-statsmodels`, `python3-statsmodels-lib`, `python3-seaborn`, `python3-joblib`, `python3-patsy`, `python3-threadpoolctl` (458 → **466**, Annexe B ; taille de l'image `image-execution.tar` 1 289 Mo → **1 399 Mo**, mesuré sur l'archive réellement produite) — empreinte nettement plus légère que `python3-geopandas` (constat 13), aucune de ces quatre bibliothèques n'entraînant de pile de pilotes externe comme GDAL. Vérification CVE de cet ajout sur le Debian Security Tracker, **6/6 nouveaux paquets source couverts** (`scikit-learn` pour `python3-sklearn`/`python3-sklearn-lib`, `statsmodels` pour `python3-statsmodels`/`python3-statsmodels-lib`, `seaborn`, `joblib`, `patsy`, `python-threadpoolctl`) : `statsmodels`, `seaborn`, `patsy` et `python-threadpoolctl` n'ont **aucune CVE répertoriée**. `scikit-learn` porte **2 CVE ouvertes** — **CVE-2024-5206** (`TfidfVectorizer` conserve par erreur tous les tokens d'entraînement, y compris potentiellement sensibles, dans son attribut `stop_words_` ; exploitable seulement par qui a accès à un objet vectorizer entraîné **persisté et exposé** — SILLON ne persiste ni n'expose aucun objet Python entre jobs ou utilisateurs, chaque script s'exécute dans un conteneur éphémère détruit en fin de job, §7.7) et **CVE-2020-13092** (désérialisation de données non fiables permettant l'exécution de code — même famille que `pandas.read_pickle()`/CVE-2020-13091 déjà documentée au constat 12). `joblib` porte **1 CVE ouverte** — **CVE-2024-34997** (désérialisation dans `joblib.numpy_pickle.NumpyArrayWrapper.read_array()`, contestée par l'éditeur amont comme limitée à du contenu de cache réputé fiable). Les 3 CVE ouvertes désamorcées par le même raisonnement que les constats 12/13 : un script utilisateur dispose déjà d'une exécution de code arbitraire dans son propre conteneur isolé, donc aucune ne lui donne un accès qu'il n'ait déjà. | **Vendorisé, déployé et vérifié en conditions réelles (17/09/2026)** | `sillon-image-execution`, `sillon-worker`, `sillon-tutoriel`, `sillon-server` (`librairies-scripts.json`) | Image reconstruite (`docker build --provenance=false --sbom=false`, `podman` indisponible sur la machine de développement — mêmes options de substitution que `build/build.sh`, voir constat 1). Deux nouveaux scripts d'exemple ajoutés au tutoriel (`exemple_2.7_typologie_scikit_learn.py` : `StandardScaler` + `KMeans`, typologie en 4 profils de communes ; `exemple_2.8_regression_statsmodels_seaborn.py` : régression OLS `statsmodels` + visualisations `seaborn`), validés d'abord hors SILLON (mêmes calculs rejoués directement sur le CSV source du tutoriel, sans base PostgreSQL, pour isoler la logique Python de l'intégration SILLON), puis **installés et vérifiés en conditions réelles sur la VM de test `192.168.122.80`** (mise à niveau depuis `sillon-image-execution` 0.1.3/`sillon-tutoriel` 0.2.3 déjà en place sur cette VM depuis le constat 13, `podman load` réussi) : les 8 scripts d'exemple du tutoriel, dont les deux nouveaux (jobs n°26 et 27), exécutés réellement par ce worker contre la base PostgreSQL du compte `demo@sillon.local`, tous terminés avec succès — résultats téléchargés et comparés octet pour octet aux fichiers `.csv`/`.txt` de la vérification hors SILLON, identiques (R² = 0,342, mêmes 4 profils de clustering, mêmes résidus par département). Les 8 nouveaux paquets ont chacun été couverts par la vérification CVE (via leur paquet source). Reste à faire : resynchronisation de `sbom/sillon-sbom-cyclonedx.json` (comme différée puis rattrapée au constat 13, entrée 15 du journal) et report de ces versions dans les annexes A/B/C, résolues sur une VM différente (`192.168.122.114`, §2) |

## 8. Veille des vulnérabilités

**Paquets Debian** — Les 798 paquets Debian de cette nomenclature (§9, §10, §11 — 790 avant l'ajout de scipy/scikit-learn/statsmodels/seaborn le 17/09/2026 (constat 14), 710 avant l'ajout de `python3-geopandas` le 16/09/2026, constat 13) sont, par construction, tous suivis par le [Debian Security Tracker](https://security-tracker.debian.org/tracker/) via leur paquet source (colonne « Paquet source Debian » des tableaux ci-dessous). Rapprocher périodiquement `sbom/sillon-sbom-cyclonedx.json` de ce suivi — ou d'un scanner tel que `grype`/`trivy` exécuté directement contre la VM cible — constitue la mécanique de veille recommandée pour couvrir l'obligation de gestion des vulnérabilités du règlement NIS2.
...
**Extension du 17/09/2026, vendoring de `python3-scipy`/`python3-sklearn`/`python3-statsmodels`/`python3-seaborn` (constat 14, §7)** — les 8 nouveaux paquets Debian (6 paquets source uniques : `scikit-learn`, `statsmodels`, `seaborn`, `joblib`, `patsy`, `python-threadpoolctl`), couverts à 100 % individuellement sur le Debian Security Tracker : `statsmodels`, `seaborn`, `patsy` et `python-threadpoolctl` sans CVE ; `scikit-learn` avec 2 CVE ouvertes (fuite de tokens via `TfidfVectorizer.stop_words_`, inatteignable en l'absence de tout objet Python persisté/partagé entre jobs dans SILLON ; désérialisation non fiable, même famille que `pandas.read_pickle()` au constat 12) ; `joblib` avec 1 CVE ouverte (désérialisation dans `NumpyArrayWrapper`, portée contestée par l'éditeur amont). Les 3 désamorcées par le même raisonnement que les constats 12/13.
...
## 10. Annexe B — Paquets Debian résolus, univers image d'exécution (466)
...
| `python3-joblib` | 1.4.2-4 | `joblib` | all |
...
| `python3-patsy` | 1.0.1-1 | `patsy` | all |
...
| `python3-seaborn` | 0.13.2-5 | `seaborn` | all |
| `python3-shapely` | 2.1.0-1 | `python-shapely` | amd64 |
| `python3-sklearn` | 1.4.2+dfsg-8 | `scikit-learn` | all |
| `python3-sklearn-lib` | 1.4.2+dfsg-8 | `scikit-learn` | amd64 |
| `python3-statsmodels` | 0.14.4+dfsg-1 | `statsmodels` | all |
| `python3-statsmodels-lib` | 0.14.4+dfsg-1+b4 | `statsmodels` | amd64 |
| `python3-sympy` | 1.13.3-5 | `sympy` | all |
| `python3-threadpoolctl` | 3.1.0-1 | `python-threadpoolctl` | all |
...
- **Scan de vulnérabilités connues (CVE) sur les paquets Debian, très partiel** : outre `postgresql-17`/`nginx` (univers hôte, constat 11), les 5 bibliothèques Python de l'univers image d'exécution exposées aux scripts utilisateurs (constat 12), les 80 paquets ajoutés avec `python3-geopandas` (constat 13) et les 8 paquets ajoutés avec scipy/scikit-learn/statsmodels/seaborn (constat 14) confrontés individuellement au Debian Security Tracker, et les 10 composants vendorisés hors `dpkg` (§6.1) — les ~695 paquets Debian restants listés en annexe (dont R/tidyverse dans l'image d'exécution) n'ont pas été confrontés un à un à ce suivi — voir §8 pour la mécanique de veille recommandée sur ce périmètre.
- **SBOM CycloneDX resynchronisé le 17/09/2026** (journal, entrée 15) : les 80 paquets ajoutés par `python3-geopandas` (constat 13), les 13 dérives de version associées et les 4 étiquettes de version de paquets SILLON (`sillon-server`, `sillon-orchestrateur`, `sillon-image-execution`, `sillon-tutoriel`) sont désormais reflétés dans `sbom/sillon-sbom-cyclonedx.json` (818 composants), revalidé sans erreur contre le schéma officiel CycloneDX 1.6. Les 14 CVE non corrigées de `gdal`/`netcdf` (constat 13) y sont documentées comme `not_affected` (VEX), avec la même justification que dans ce document. **Reste non repris** : les dépendances Debian propres à `sillon-orchestrateur`/`sillon-tutoriel` au-delà de ces étiquettes de version n'ont pas été réauditées à cette occasion (aucun nouveau paquet Debian constaté pour ces deux paquets depuis leur dernière résolution complète, donc sans impact connu) ; **et surtout**, les 8 paquets ajoutés le 17/09/2026 (constat 14, journal entrées 16-17) n'y figurent pas encore — resynchronisation restant à faire, voir §13.
- **`sillon-image-execution` 0.1.4, `sillon-worker` 0.1.3, `sillon-tutoriel` 0.2.4 et `sillon-server` 0.1.40 installés et vérifiés sur une VM différente de celle des annexes A/B/C** (constats 13-14) : `192.168.122.80`, pas `192.168.122.114` (§2) — podman, `sillon-worker` et `sillon-image-execution` y ont été installés pour la première fois lors du constat 13. Le bon fonctionnement applicatif de bout en bout y est confirmé (8 scripts d'exemple du tutoriel exécutés avec succès, dont les deux nouveaux du constat 14 — jobs n°26/27), mais cette VM ne remplace pas `192.168.122.114` comme référence de résolution des annexes A/B/C de ce document.
...
- Resynchroniser `sbom/sillon-sbom-cyclonedx.json` avec les 8 paquets ajoutés par scipy/scikit-learn/statsmodels/seaborn (constat §7.14, déployé et vérifié sur `192.168.122.80` le 17/09/2026, Annexe B déjà mise à jour dans ce document mais pas encore reportée dans le SBOM machine-lisible) — même reste à faire que le point précédent, à traiter ensemble lors de la prochaine resynchronisation.
