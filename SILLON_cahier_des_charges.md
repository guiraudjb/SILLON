...
En complément du dépôt manuel, un bouton « Importer depuis data.gouv.fr » ouvre une modale de recherche dans le catalogue public de data.gouv.fr (recherche par mot-clé, appel direct à l'API de recherche de data.gouv.fr depuis l'orchestrateur, éventuellement via le proxy sortant configuré en §5.6). Pour chaque jeu de données trouvé, sont proposées à l'import les ressources déclarées aux formats CSV, TXT, JSON ou ZIP (avec leur taille si l'API la communique) — un recensement réel sur data.gouv.fr a montré que le CSV seul ne représentait qu'environ 17 % des ressources rencontrées, contre par exemple le jeu « Demandes de valeurs foncières » publié en `.txt.zip`. Le géospatial (GeoJSON, Shapefile, WMS/WFS...) reste hors périmètre. Un format non CSV est normalisé côté serveur avant l'aperçu (extraction d'archive ZIP, aplatissement JSON à un niveau — GeoJSON explicitement rejeté) : le reste du pipeline d'import (assistant de relecture des colonnes, contrôle de cohérence, chargement) ne voit jamais que du CSV, quelle que soit l'origine du fichier.

Les ressources déclarées au format PDF du même jeu de données sont proposées séparément, comme documentation facultative : le choix d'un PDF (optionnel, indépendant du choix de la ressource de données) l'attache à la table lors de sa création, consultable depuis sa fiche (§5.2). Cette association n'existe que pour ce flux data.gouv.fr, pas pour un dépôt manuel.

Le choix d'une ressource de données déclenche son téléchargement côté serveur (avec le même garde-fou de taille que le dépôt manuel, §11 — une limite dédiée, plus généreuse, pour la documentation PDF), avec un retour de progression pendant le téléchargement (volume reçu, volume total si connu, vitesse), puis rouvre exactement le même assistant qu'un dépôt de fichier (étapes 2 à 9 ci-dessus, sans distinction ensuite entre les deux origines).
...
- Trois paquets séparés, reflétant une architecture à deux machines : `sillon-backup-client` s'installe sur le serveur SILLON (dépend de `sillon-server` et `pgbackrest`, configure le montage automatique du partage NFS distant, l'archivage continu des journaux de transaction PostgreSQL et la tâche planifiée quotidienne) ; `sillon-backup-server` s'installe sur une machine de sauvegarde distincte (dépend de `nfs-kernel-server` et `pgbackrest`, expose un partage NFS restreint par adresse IP au seul serveur SILLON) ; `sillon-backup-server-survey`, optionnel, s'installe sur cette même machine de sauvegarde (dépend de `sillon-backup-server`, `mailutils` et `postfix`) et vérifie quotidiennement, sans avoir besoin de contacter le serveur SILLON, l'état des sauvegardes présentes sur le partage. Chaque paquet demande son unique paramètre de site (adresse du serveur de sauvegarde, adresse autorisée à y déposer, destinataires des alertes et relais SMTP) via un menu interactif (`debconf`) à l'installation, modifiable ensuite sans réinstallation (`dpkg-reconfigure`).
- **Restauration à une sauvegarde choisie** : l'administrateur sélectionne un numéro de ligne dans une liste des sauvegardes disponibles, classées de la plus récente à la plus ancienne, plutôt que de saisir une date/heure ; la solution de sauvegarde détermine elle-même la sauvegarde complète et la chaîne de sauvegardes incrémentales nécessaires jusqu'à ce point précis — rien à sélectionner manuellement au-delà du numéro. L'ensemble du cluster (catalogue et toutes les bases agents) est ramené à cet état. Aucune limite de durée n'est imposée (une restauration volumineuse peut prendre plusieurs heures), la progression de chaque étape (copie des fichiers, rejeu des journaux) est affichée en continu. Deux catégories de secrets applicatifs, dupliqués hors base de données (§12.3), sont resynchronisées automatiquement à l'issue de la restauration, dans des sens opposés : le secret de signature des jetons est repris depuis la base restaurée vers ce serveur (PostgREST doit vérifier avec le même secret que celui utilisé pour signer) ; les mots de passe des rôles techniques PostgreSQL (API de requêtage, orchestrateur) sont au contraire réimposés depuis ce serveur vers les rôles restaurés — condition nécessaire à une restauration sur une **installation neuve** (reconstruction après sinistre), où ces mots de passe ont été générés aléatoirement à l'installation et diffèrent de ceux de la sauvegarde d'origine.
